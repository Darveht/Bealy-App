...
// Modificar la clase AppAnimationSystem para incluir Netflix
class AppAnimationSystem {
  constructor() {
    this.isAnimating = false;
    this.plantApps = ['Roblox', 'Among Us', 'Candy Crush Saga']; // Apps que tendrán animación de planta
  ...
  async showAnimation(appName, clickedElement) {
    ...
    if (!hasCustomAnimation) {
      ...
      if (animationType === 'plant') {
        ...
        this.showTikTokAnimation();
      } else if (animationType === 'netflix') {
        this.showNetflixAnimation();
      }
    }
  ...
  async showNetflixAnimation() {
    // Crear el contenedor de animación Netflix
    const animationContainer = document.createElement('div');
    animationContainer.className = 'netflix-animation-container';
    ...
    animationContainer.innerHTML = `
      <div class="netflix-scene">
        <div class="wednesday-logo-container">
          <img src="https://static.wikia.nocookie.net/timburton/images/8/81/Wednesday_Logo.png/revision/latest?cb=20230902200434" alt="Wednesday Logo" class="wednesday-logo">
        </div>
        <div class="netflix-particles">
        ...
        <div class="dark-atmosphere"></div>
      </div>
    `;

    document.body.appendChild(animationContainer);

    // Ejecutar animación Netflix
    await this.executeNetflixSequence(animationContainer);
  ...
  async executeNetflixSequence(container) {
    const logo = container.querySelector('.wednesday-logo');
    const particles = container.querySelectorAll('.particle');
    const atmosphere = container.querySelector('.dark-atmosphere');

    // Activar la atmósfera oscura inmediatamente
    atmosphere.classList.add('active');

    // Logo aparece desde abajo con delay de 0.5s
    setTimeout(() => {
      logo.classList.add('rising');
    }, 500);

    // Partículas aparecen con delay
    setTimeout(() => {
    ...
    // Logo se desvanece después de 4 segundos
    setTimeout(() => {
      logo.classList.add('fading');
      atmosphere.classList.add('fading');
      particles.forEach(particle => {
        particle.classList.add('disappearing');
      });
    }, 4000);
  }
